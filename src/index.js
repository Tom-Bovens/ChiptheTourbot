...
const errorCatch = (error, line) => {
    log("A promise has errored. You can find it's location in the index.js file with the following trace.")
    log(line)
    log(error);
}
...
const nextVideoinPath = (conversation, currentIteration = 0, videoContentDescription, userId, specialMessage) => {
    ...
    bot.send(conversation.id, messages);
}
    ...
    try {
        ...
        switch (message.text) {
            ...
            case "CancelTour":
                ...
                break
            case "0complete":
                nextVideoinPath(conversation, 1, "the different statusses of conversations.", userId, "By the way. You can open the tour menu by starting a new conversation with me via the dashboard..")
                break
            case "1complete":
